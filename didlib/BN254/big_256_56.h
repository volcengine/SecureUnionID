 ...
 *
	Assumes that the random number generator has been suitably initialised
	@param m BIG number, on exit a random number
	@param q The modulus
	@param nonce 64 bytes nonce value
 */
void BIG_256_56_randnum(BIG_256_56 m, BIG_256_56 q, unsigned char *nonce);

/**	@brief  Create an unbiased random BIG from a seed, reduced with respect to a modulus
 *
	Assumes that the random number generator has been suitably initialised
	@param m BIG number, on exit a random number
	@param q The modulus
