...
int BIG_256_56_fshl(BIG_256_56 a, int n) {
    ...
    a[NLEN_256_56 - 1] =
            ((a[NLEN_256_56 - 1] << n)) | (a[NLEN_256_56 - 2] >> (BASEBITS_256_56 - n)); /* top word not masked */
    for (i = NLEN_256_56 - 2; i > 0; i--)
        a[i] = ((a[i] << n) & BMASK_256_56) | (a[i - 1] >> (BASEBITS_256_56 - n));
    a[0] = (a[0] << n) & BMASK_256_56;

    return (int) (a[NLEN_256_56 - 1]
            >> ((8 * MODBYTES_256_56) % BASEBITS_256_56)); /* return excess - only used in ff.c */
}
...
//////////////////
/* get 8*MODBYTES size random number */
/*void BIG_256_56_rand(BIG_256_56 m, unsigned char *nonce) {
    int i, b, j = 0;
    unsigned char r = 0;
    int len = 8 * MODBYTES_256_56;
    ...
    *//* generate random BIG *//*
    for (i = 0; i < len; i++) {
        if (j == 0) random_data(&r, 1, nonce);//r=RAND_byte(rng);
        else r >>= 1;
        b = r & 1;
        BIG_256_56_shl(m, 1);
        m[0] += b;
...
/* get 8*MODBYTES size random number */
/*void BIG_256_56_seed(BIG_256_56 m, unsigned char *seed, int seedLen) {
    int i, b, j = 0;
    unsigned char r = 0;
    int len = 8 * MODBYTES_256_56;
    ...
    *//* generate random BIG *//*
    for (i = 0; i < len; i++) {
        if (j == 0) KDF(&r, 1, seed, seedLen, nonce);//random_data(r, 1);//r=RAND_byte(rng);
        else r >>= 1;
        b = r & 1;
        BIG_256_56_shl(m, 1);
        m[0] += b;
...
/////////////////////////
/* get random BIG from rng, modulo q. Done one bit at a time, so its portable */
/*void BIG_256_56_randnum(BIG_256_56 m, BIG_256_56 q, unsigned char *nonce) {
    int i, b, j = 0;
    unsigned char r = 0;
    DBIG_256_56 d;
    BIG_256_56_dzero(d);
    *//* generate random DBIG *//*
    for (i = 0; i < 2 * BIG_256_56_nbits(q); i++) {
        if (j == 0) random_data(&r, 1, nonce);//RAND_byte(rng);
        else r >>= 1;
        b = r & 1;
        BIG_256_56_dshl(d, 1);
        d[0] += b;
...
/* get random BIG from rng, modulo q. Done one bit at a time, so its portable */
/*void BIG_256_56_seednum(BIG_256_56 m, BIG_256_56 q, unsigned char *seed, int seedLen) {
    int i, b, j = 0;
    unsigned char r = 0;
    DBIG_256_56 d;
    unsigned char nonce[32]={0};
    ...
    *//* generate random DBIG *//*
    for (i = 0; i < 2 * BIG_256_56_nbits(q); i++) {
        if (j == 0) {KDF(&r, 1, seed, seedLen, nonce);}//RAND_byte(rng);
        else r >>= 1;
        b = r & 1;
        BIG_256_56_dshl(d, 1);
        d[0] += b;
