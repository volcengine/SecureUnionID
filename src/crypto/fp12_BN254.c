...
static int FP12_teq(sign32 b, sign32 c) {
    sign32 x = b ^c;
    x -= 1;  // if x=0, x now -1
    return (int) ((x >> 31) & 1);
}
...
static void FP12_BN254_select(FP12_BN254 *f, FP12_BN254 g[], sign32 b) {
    ...
    FP12_BN254_cmove(f, &g[0], FP12_teq(babs, 0));  // conditional move
    FP12_BN254_cmove(f, &g[1], FP12_teq(babs, 1));
    FP12_BN254_cmove(f, &g[2], FP12_teq(babs, 2));
    FP12_BN254_cmove(f, &g[3], FP12_teq(babs, 3));
    FP12_BN254_cmove(f, &g[4], FP12_teq(babs, 4));
    FP12_BN254_cmove(f, &g[5], FP12_teq(babs, 5));
    FP12_BN254_cmove(f, &g[6], FP12_teq(babs, 6));
    FP12_BN254_cmove(f, &g[7], FP12_teq(babs, 7));
...
void FP12_BN254_ssmul(FP12_BN254 *w, FP12_BN254 *y) {
    ...
    if (y->type >= AMCL_FP_SPARSE) {
...
#endif
        FP4_BN254_mul(&z2, &(w->b),
                      &(y->b));  // xb.yb  could be 00x00 or 01x01 or or 10x10 or 11x00 or 11x10 or 11x01 or 11x11
        ...
        FP4_BN254_add(&(w->c), &z2,
                      &t1);        // wc = (xb.yb) - (xa.ya) + (xa+xc)(ya+yc) - (xc.yc)	=  xb.yb + xc.ya + xa.yc
...
void FP12_BN254_compow(FP4_BN254 *c, FP12_BN254 *x, BIG_256_56 e, BIG_256_56 r) {
    FP12_BN254 g1, g2;
...
void FP12_BN254_pow(FP12_BN254 *r, FP12_BN254 *a, BIG_256_56 b) {
    FP12_BN254 w, sf;
    BIG_256_56 b1, b3;
    int i, nb, bt;
    BIG_256_56_copy(b1, b);
    BIG_256_56_norm(b1);
    BIG_256_56_pmul(b3, b1, 3);
    BIG_256_56_norm(b3);
    ...
    for (i = nb - 2; i >= 1; i--) {
        FP12_BN254_usqr(&w, &w);
        bt = BIG_256_56_bit(b3, i) - BIG_256_56_bit(b1, i);
        if (bt == 1)
            FP12_BN254_mul(&w, &sf);
        if (bt == -1) {
...
void FP12_BN254_pow4(FP12_BN254 *p, FP12_BN254 *q, BIG_256_56 u[4]) {
    int i, j, k, nb, pb, bt;
    FP12_BN254 g[8], r;
    BIG_256_56 t[4], mt;
